...
<!--          <q-btn flat round dense icon="qr_code" @click="qrPrint(props.row)" />-->
          <q-btn flat round dense icon="add_circle_outline" @click="cupoRegisterDirect(props.row)" />
    ...
    <q-dialog v-model="registerDialog" persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          <q-form>
            <q-input v-model="cupo.nombre" label="Nombre" />
            <q-btn @click="cupoRegister(cupo)" label="Registrar" />
          </q-form>
        </q-card-actions>
      </q-card>
    </q-dialog>
...
export default {
  ...
  methods: {
    ...
    cupoRegisterDirect(cupo){
      this.$q.loading.show()
      this.$api.post(this.url+'cupoRegisterDirect',cupo).then(response => {
